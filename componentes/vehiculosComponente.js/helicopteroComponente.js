...
export default class Helicoptero extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: true,
  ...
  componentDidMount() {
    return fetch('https://gtavehicles.000webhostapp.com/api/api-helicopteros.php')
  ...
  render() {
    ...
    if (this.state.isLoading) {
      return (
        <View>
           <CustomHeader titulo="Helicopteros" navigation={this.props.navigation} />
      ...
      let dataInformacion = this.state.data.map((val, key) => {
        ...
        return <List key = {val.idvehiculo} >
